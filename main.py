...
def expected_change():
    ...
    for interval in forecast['list']:
        ...
        if forecast_weather != current_weather:            
            time_str = forecast_time.split(" ")[1].split(":")[0]
            hour = int(time_str)
...
temp = int(round(weather_now['main']['temp'],0))
feel_like = int(round(weather_now['main']['feels_like'],0))
min_temp = int(round(min([i['main']['temp_min'] for i in forecast['list']]), 0))
max_temp = int(round(max([i['main']['temp_max'] for i in forecast['list']]), 0))
